...
class Gridworld(VectorizedTask):
    ...
    def __init__(self,
        ...
        self.obs_shape = tuple([(AGENT_VIEW*2+1)*(AGENT_VIEW*2+1)*5*2+6*2, ])
        ...
        def reset_fn(key):
            ...
            return State(state=grid, obs=jnp.concatenate([get_obs(state=grid,posx=posx,posy=posy),jnp.zeros(6),get_obs(state=grid,posx=posx,posy=posy),jnp.zeros(6)]),last_action=jnp.zeros((7,)),reward=jnp.zeros((1,)),agent=agent,
        ...
        def step_fn(state, action):
            ...
            grid=state.state
            reward=0
            prev_obs=jnp.concatenate([get_obs(state=state.state,posx=state.agent.posx,posy=state.agent.posy),jax.nn.one_hot(state.agent.inventory,6)])
            #move agent
            ...
            cur_state=State(state=grid, obs=jnp.concatenate([get_obs(state=grid,posx=posx,posy=posy),jax.nn.one_hot(inventory,6),prev_obs]),last_action=action,reward=jnp.ones((1,))*reward,agent=AgentState(posx=posx,posy=posy,inventory=inventory),
