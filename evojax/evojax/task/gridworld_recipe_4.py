...
def get_obs(state: jnp.ndarray,posx:jnp.int32,posy:jnp.int32) -> jnp.ndarray:
    obs=jnp.ravel(jax.lax.dynamic_slice(jnp.pad(state,((AGENT_VIEW,AGENT_VIEW),(AGENT_VIEW,AGENT_VIEW),(0,0))),(posx-AGENT_VIEW+AGENT_VIEW,posy-AGENT_VIEW+AGENT_VIEW,1),(2*AGENT_VIEW+1,2*AGENT_VIEW+1,6)))
    return obs

def get_init_state_fn(key: jnp.ndarray) -> jnp.ndarray:
    grid=jnp.zeros((SIZE_GRID,SIZE_GRID,7))
    posx,posy=(1,1)
    ...
    next_key, key = random.split(next_key)
    perm=jax.random.randint(key,(3,),0,SIZE_GRID-1)
    next_key, key = random.split(next_key)
    perm2=jax.random.randint(key,(1,),0,SIZE_GRID)
    #pos_obj=jnp.array([[perm[0],perm2[0]],[perm[1],perm2[1]],[perm[2],perm2[2]]])
    pos_obj=jnp.array([[perm[0],0],[perm[1],1],[perm[2],2],[2,perm2[0]]])
...
def test_recipes(items,recipes):
	...
	reward=jnp.select([product==0,product==5,product==6],[0.,1.,2.])
...
class Gridworld(VectorizedTask):
    ...
    def __init__(self,
        ...
        self.obs_shape = tuple([(AGENT_VIEW*2+1)*(AGENT_VIEW*2+1)*6+7, ])
        ...
        def reset_fn(key):
            ...
            next_key, key = random.split(next_key)
            permutation_recipe=jax.random.permutation(key,4)[:3]+1
            ...
            return State(state=grid, obs=jnp.concatenate([get_obs(state=grid,posx=posx,posy=posy),jnp.zeros(7)]),last_action=jnp.zeros((7,)),reward=jnp.zeros((1,)),agent=agent,
        ...
        def step_fn(state, action):
            ...
            steps = state.steps + 1
            done = jnp.logical_or(grid[:,:,-2].sum()>0 ,steps>self.max_steps)
            ...
            cur_state=State(state=grid, obs=jnp.concatenate([get_obs(state=grid,posx=posx,posy=posy),jax.nn.one_hot(inventory,7)]),last_action=action,reward=jnp.ones((1,))*reward,agent=AgentState(posx=posx,posy=posy,inventory=inventory),
